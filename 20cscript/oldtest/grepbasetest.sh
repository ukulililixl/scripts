...
RES=/home/ncsgroup/xiaolu/hadoopctdrlogs/greplogs/res_${K}_${M}_${BLOCKSIZE}_${USECTDR}_${POLICY}_${RUN}
HADOOPHOME=/home/ncsgroup/xiaolu/hadoop-20-ctdr

echo "start k=$K m=$M blocksize=${BLOCKSIZE}MB usectdr=$USECTDR policy=$POLICY for the $RUN time of grep job" >$RES
date >>$RES
...
# 12 start grep
echo "start $i grep" >>$RES
JAR=/home/ncsgroup/xiaolu/hadoop-20-ctdr/build/hadoop-0.20-examples.jar
hadoop jar $JAR grep raidTest/input1 output$i 'repair' -D mapred.reduce.tasks=0 >>$RES 2>&1
echo "stop $i grep" >>$RES
stop-mapred.sh
done

# 15 backup logs
HADOOPLOG=/home/ncsgroup/xiaolu/hadoopctdrlogs/greplogs
...
ssh cloud-node$i "cp -r /home/ncsgroup/CTDR/output ${CTDRLOG}/grepbaseoutput_${K}_${M}_${BLOCKSIZE}_${USECTDR}_${POLICY}_${RUN}"
done 
