...
do
	echo i=$i
        ...
        servername="${server%%:*}"
        serverip="${server##*:}"
        log=/home/xiaolu/iperflog/${servername}_${testnum}
        echo server=$servername $serverip >$log
        ssh $serverip "/home/xiaolu/iperftest/iperftest.sh $servername $serverip 1 $testnum"

	client=${array2[i]}
	clientname="${client%%:*}"
	clientip="${client##*:}"
	echo inner-region client=$clientname $clientip >>$log
	ssh $clientip "/home/xiaolu/iperftest/iperftest.sh $servername $serverip 0 $testnum" >>$log
	
	next=`expr $i + 1`
	for j in `seq ${next} ${max}`  	
	do
        	client=${array[j]}
       		clientname="${client%%:*}"
        	clientip="${client##*:}"
        	echo client=$clientname $clientip >>$log
        	ssh $clientip "/home/xiaolu/iperftest/iperftest.sh $servername $serverip 0 $testnum" >>$log
	done
        ssh $serverip "pkill -f iperf3"
done
